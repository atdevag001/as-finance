...
// Seed credentials for field_officer role
...
/**
 * Helper: log in as field_officer and navigate to the new customer form.
 */
async function loginAndGoToNewCustomer(page: Page) {
  await page.goto('/login');
  await page.getByLabel('Username').fill(FO_USERNAME);
  await page.getByLabel('Password').fill(FO_PASSWORD);
 ...
 */
async function fillValidCustomerForm(page: Page, overrides: Partial<CustomerFormData> = {}) {
  ...
  if (data.fatherOrHusbandName) {
    await page.getByLabel('Father/Husband Name').fill(data.fatherOrHusbandName);
  }
  await page.getByLabel('Mobile *').fill(data.mobile);
  await page.getByLabel('Aadhaar Number *').fill(data.aadhaarNumber);

  // Select gender from dropdown
  await page.getByLabel('Gender *').selectOption(data.gender);

  await page.getByLabel('Address *').fill(data.addressLine1);
...
test.describe('Customer Onboarding', () => {
  test.beforeEach(async ({ page }) => {
    await loginAndGoToNewCustomer(page);
  });
  ...
  test('Aadhaar validation error shown inline for invalid format', async ({ page }) => {
    ...
    await fillValidCustomerForm(page, { aadhaarNumber: '12345' });
    ...
    await expect(page.getByText(/12-digit Aadhaar/i)).toBeVisible();
  });

  test('PAN validation error shown inline for invalid format', async ({ page }) => {
    // Check if PAN field exists on the form
    const panField = page.getByLabel(/PAN/i);
    const panFieldVisible = await panField.isVisible().catch(() => false);
  ...
  test('KYC document upload with valid file → verify upload success', async ({ page }) => {
    const fileInput = page.locator('input[type="file"]');
    const fileInputExists = await fileInput.count();

    if (fileInputExists === 0) {
      // KYC upload is not present on the current customer creation form
      test.skip();
      return;
    }
  ...
  test('KYC upload with invalid MIME type → verify error message', async ({ page }) => {
    const fileInput = page.locator('input[type="file"]');
    const fileInputExists = await fileInput.count();

    if (fileInputExists === 0) {
      // KYC upload is not present on the current form
      test.skip();
      return;
    }
