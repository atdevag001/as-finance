...
export interface CreateCustomerData {
  // Pre-generated by the service so the AAD bound at encrypt time matches the row id.
  id?: string;
  full_name: string;
...
export class CustomerRepository {
  ...
  async update(id: string, data: UpdateCustomerData, expectedVersion?: number) {
    ...
    // Optimistic concurrency: when caller supplies expectedVersion, guard the
    // write so concurrent edits cannot silently overwrite each other.
    if (expectedVersion !== undefined) {
      const result = await this.prisma['customers'].updateMany({
      ...
      if (result.count === 0) {
        throw new ConflictError(
          'Customer was modified by another request. Please reload and retry.',
